...
function AnswerEffectEditor(props: AnswerEffectEditorProps) {
    ...
    function deleteAnswerEffect() {
        ...
        if (!window.confirm(
            ...
            "Tüm etkiler (amount, candidateId, issueId vb.) kalıcı olarak kaybolacak.\n\n" +
    ...
    if (issues.length > 0) {
        ...
        const remappedIssueScore = Math.min(
            Math.floor(((answerEffect.amount + 1) / 2) * issue.stances.length),
    ...
    return (
        <div className="EditorBox AnswerEffectEditor">
            <div>
                ...
                <select
                    onChange={(e) => updateFieldAndUpdateData<string>("answerEffectType", e.target.value)}
                    value={answerEffect.answerEffectType}
                >
                    {answerEffectTypes.map((effectType) => (
            ...
            {(typedAnswerEffectType !== AnswerEffectType.SetQuestionEnabled) && (
                <div>
                    ...
                    <select
                        onChange={(e) => updateFieldAndUpdateData<number>("candidateId", Number(e.target.value))}
                        value={answerEffect.candidateId}
                    >
            ...
            {(typedAnswerEffectType === AnswerEffectType.State) && (
                <div>
                    ...
                    <select
                        onChange={(e) => updateFieldAndUpdateData<number>("stateId", Number(e.target.value))}
            ...
            {(typedAnswerEffectType === AnswerEffectType.Issue || typedAnswerEffectType === AnswerEffectType.TctIssue) && (
                <div>
                    ...
                    <select
                        onChange={(e) => updateFieldAndUpdateData<number>("issueId", Number(e.target.value))}
            ...
            {(typedAnswerEffectType !== AnswerEffectType.SetQuestionEnabled && typedAnswerEffectType !== AnswerEffectType.TctIssue) && (
                <div>
                    <GenericEditorInput
                        label={"Amount"}
                        type={"number"}
                        defaultValue={answerEffect.amount}
                        onChange={(e) => updateFieldAndUpdateData("amount", Number(e.target.value))}
                    />
                </div>
            )}
            ...
            {(typedAnswerEffectType === AnswerEffectType.TctIssue) && (
                <div>
                    <GenericEditorInput
                        label={"Weight"}
                        type={"number"}
                        defaultValue={answerEffect.weight ?? 1.0}
                        onChange={(e) => updateFieldAndUpdateData("weight", Number(e.target.value))}
                    />
                </div>
            )}
            ...
            <button
                title="Clone answer effect"
                className="CircleButton BlueButton"
                onClick={cloneAnswerEffect}
            >
                📄
            </button>
        </div>
